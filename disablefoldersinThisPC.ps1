...
function set-folderHidden($guid, $hide) {
 ...
 # 
   ...
   Write-Host "$null = new-itemProperty -path $regPath -name $guid  -value $hide  -force"
   ...
   if (($hide -eq 1) -and (test-path $regPath64)) {
        Write-Host "$null = remove-item -path $regPath64 -force"
        $null = remove-item -path $regPath64 -force
   }
   if ($hide -eq 0) {
    Write-Host "$null = new-item -path $regPath64 -force"
    $null = new-item -path $regPath64 -force
    }
}
